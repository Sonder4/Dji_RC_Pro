...
`ControllerState -> PID=0x09 -> UDP -> gateway -> /joy -> joy_to_cmd_vel -> /cmd_vel`

### `ble_only`

`ControllerState -> PID=0x09 -> BLE -> gateway -> /joy -> joy_to_cmd_vel -> /cmd_vel`

### `ble_udp`

`ControllerState -> PID=0x09 -> UDP 优先，BLE 回退 -> gateway -> /joy -> joy_to_cmd_vel -> /cmd_vel`
...
2. ROS2 单元测试验证原始输入解码、`Joy` 归一化和按键展开。
3. ROS2 构建后验证网关与 Joy 映射节点均可编译安装。
4. 启动 `dji_rc_pro_gateway.launch.py` 时，确认 `gateway + joy_to_cmd_vel` 同时被拉起。

## 回退策略

如果原始输入主链路在现场验证中出现问题，可临时切回旧 Android 发送逻辑，ROS2 网关仍能接收 `PID=0x01`。这保证本次迁移是“可前进、可回退”的。
