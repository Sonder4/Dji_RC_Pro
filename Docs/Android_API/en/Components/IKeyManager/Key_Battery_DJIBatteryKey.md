...
`DJIBatteryKey` provides a set of methods to manage battery information and real-time status. Obtain the information and real-time status of the corresponding battery by setting componentIndex. 0 means battery 1 in the Pilot, 1 means battery 2 in the Pilot. `This class is supported since MSDK 5.0.`
