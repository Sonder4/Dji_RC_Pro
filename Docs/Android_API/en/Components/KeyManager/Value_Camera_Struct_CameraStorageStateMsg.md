...
|  |
| --- |
| ``` class CameraStorageInfos implements DJIValue, JNIProguardKeepTag, ByteStream ``` |
...
|  |
| --- |
| ``` List<CameraStorageInfo> getStorageInfos() ``` |
...
To get the details of the current camera storage card, including the details of SD Card and onboard memory.

##### Return:

|  |  |
| --- | --- |
| List<[CameraStorageInfo](../../Components/KeyManager/Value_Camera_Struct_CameraStorageState.md#value_camera_struct_camerastoragestate)> | *Returns details of the current camera storage card.* |
