...
- 默认轴映射 `yaw<-axes[2]`
- 按下加速键 `R1(button 5)` 时使用 turbo 缩放
- 轴越界或按钮缺失时输出安全的零速度

测试固定采用如下规则：
- `enable_button = 4`
- `enable_turbo_button = 5`
- 默认 `require_enable_button = false`
- turbo 测试中显式设置 `scale_chassis_turbo.x = 2.0`、`scale_chassis_turbo.y = -2.0`、`scale_chassis_turbo.yaw = 3.0`
...
- `PID=0x01` 仍作为兼容接收保留
- 兼容期要求现场只启用一种发送协议，不混发 `PID=0x01` 与 `PID=0x09`
