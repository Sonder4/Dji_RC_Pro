...
Options:
  ...
  -m, --mode <udp|ble|all>     choose log filter (default: all)
...
case "$CAPTURE_MODE" in
  udp)
    COMMON_FILTER='*:S UdpService:D MainViewModel:D WifiDiscoveryManager:D DataLogManager:D AndroidRuntime:E'
    ;;
  ble)
    COMMON_FILTER='*:S BleManager:D BleService:D MainViewModel:D DataLogManager:D AndroidRuntime:E'
    ;;
  all)
    COMMON_FILTER='*:S UdpService:D BleManager:D BleService:D MainViewModel:D WifiDiscoveryManager:D DataLogManager:D AndroidRuntime:E'
    ;;
  *)
    echo "Unsupported mode: $CAPTURE_MODE" >&2
    exit 1
    ;;
esac
...
LOG_FILE="$LOG_DIR/transport_${CAPTURE_MODE}_${TIMESTAMP}.log"
META_FILE="$LOG_DIR/transport_${CAPTURE_MODE}_${TIMESTAMP}.meta"
...
if [[ "$FOLLOW_MODE" -eq 1 ]]; then
  echo "Capturing $CAPTURE_MODE transport logs to $LOG_FILE"
  exec "$ADB_BIN" -s "$DEVICE_SERIAL" logcat --pid="$APP_PID" -v threadtime $COMMON_FILTER | tee "$LOG_FILE"
else
  echo "Dumping $CAPTURE_MODE transport logs to $LOG_FILE"
